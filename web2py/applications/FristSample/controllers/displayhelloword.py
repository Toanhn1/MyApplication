...
def abc():
    return redirect(URL("formadd",vars= dict(message_email='', message='', message_age='',message_date = '')))
...
def addorder():
    ...
    for row in listOrderId:
        if order in row.order_id :
            return redirect(URL("formadd",vars= dict(message=errorKey)))
...
def doupdate():
    ...
    check_date = re.compile(regex.regrex_day)
    if not check_date.match(request.vars.ngayMua):
        return redirect(URL("error", vars=dict(message_email='', message='', message_age='',message_date = errorDate)))

    if not request.vars.age.isnumeric():
        return redirect(URL("error", vars=dict(message_email='', message='',message_age = errorAge,message_date='')))

    check_email = re.compile(regex.regrex_email)
    if not check_email.match(request.vars.email):
        return redirect(URL("error", vars=dict(message_email=errorEmail,message='',message_age='',message_date='')))

    db(db.orders.order_id == request.request.vars.orderId).update(user_id=request.vars.userId,ngay_mua=request.vars.ngayMua,
